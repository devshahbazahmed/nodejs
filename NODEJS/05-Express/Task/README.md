...
1. **Import Express**: In `index.js` file, import the Express module.
2. **Create Express App**: Use `const app = express();` to initialize the Express app.
3. **Set Up Port**: Define a constant, `const PORT = 8000;`, to store the port number.
...
1. **Define a Route**: Use `app.get('/', (req, res) => {...});` to create a route for fetching all the books.
...
3. **Receive Book Data**: Access the new book data from the `req.body`.
...
1. **Start The Server**: Run `node index.js` and check the confirmation message.
